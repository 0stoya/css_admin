...
export async function requestMagentoAdminToken(username: string, password: string) {
  const { adminTokenUrl } = getMagentoConfig();
  ...
  if (!response.ok) {
    let message = "Magento rejected the admin credentials.";
    try {
  ...
  if (typeof token !== "string" || !token.trim()) {
    throw new Error("Magento returned an invalid admin token response.");
  }
