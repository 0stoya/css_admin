...
import { companyControlsCsvImportAction } from "@/app/(admin)/companies/[id]/import-export/controls-csv-actions";
import { companyUsersCsvImportAction } from "@/app/(admin)/companies/[id]/import-export/actions";
import type {
...
const initialControlsState: CompanyControlsCsvImportState = {
  phase: "idle",
  sourceCsv: "",
  ...
  return (
    <section className="card stack import-panel">
      ...
      <div className="button-row">
        ...
        <a className="button button-secondary button-link" href={`/api/companies/${companyId}/examples/users`}>
...
export function CompanyControlsCsvImport({
  companyId,
  schemaVersion,
  roleCount,
  templateCount,
  ...
  const [state, formAction, pending] = useActionState(companyControlsCsvImportAction, initialControlsState);
    ...
    <section className="card stack import-panel">
      <div className="card-heading-row">
        <div>
          ...
          <h2>CSV controls bundle</h2>
          <p className="muted">Spreadsheet-friendly control rows are validated transactionally by Fluid before anything can be applied.</p>
        </div>
      ...
      <div className="button-row">
        ...
        <a className="button button-secondary button-link" href={`/api/companies/${companyId}/examples/controls`}>
          Download example CSV
        </a>
      </div>
      ...
      <form action={formAction} className="stack">
        ...
        <input name="intent" type="hidden" value="preview" />
        <div className="field">
          <label htmlFor="company-controls-csv">Company controls CSV</label>
          <input id="company-controls-csv" name="file" type="file" accept=".csv,text/csv" required />
        </div>
        <p className="muted small-text">CSV files open directly in Excel. Keep the record_type rows from the example so catalogue, role and purchase-template records can be reconstructed safely.</p>
