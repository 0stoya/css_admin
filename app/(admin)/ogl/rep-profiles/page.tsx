            ...
            return (
              <article className={`card ${styles.repCard} ${polish.repCard}`} key={profile.rep_code}>
                <div className={`${styles.repHeader} ${polish.repHeader}`}>
                  {profile.photo_url ? <img className={styles.photo} src={profile.photo_url} alt={`${name} profile`} loading="lazy" decoding="async" /> : <div className={styles.initials} aria-hidden="true">{initials(profile)}</div>}
                ...
                <div className={`${styles.mediaActions} ${polish.mediaActions}`}>
                  <form action={uploadOglRepPhotoAction}>
                    <input type="hidden" name="repCode" value={profile.rep_code} />
                    <input className={`${styles.fileInput} ${polish.fileInput}`} name="file" type="file" accept="image/jpeg,image/png,image/webp" required aria-label={`Choose profile photo for ${name}`} />
                    <button className="button button-secondary" type="submit" aria-label={`Upload photo for ${name}`}>Upload photo</button>
                  </form>
                  {profile.photo_url ? (
                    <form action={clearOglRepPhotoAction}>
                      <input type="hidden" name="repCode" value={profile.rep_code} />
                      <button className="button button-secondary" type="submit" aria-label={`Remove photo for ${name}`}>Remove photo</button>
                    </form>
                  ) : null}
                </div>
              </article>
