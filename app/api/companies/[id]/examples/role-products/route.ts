...
import { exampleRoleProductsCsv } from "@/lib/flat-company-imports";
...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return csvDownload(exampleRoleProductsCsv(company.reference.trim()), `company-${companyId}-role-products-example.csv`);
  } catch (error) { return adminCsvError(error, request, "Role-product example failed."); }
}
