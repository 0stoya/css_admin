...
import { exportRolesPermissionsCsv } from "@/lib/flat-company-imports";
...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return csvDownload(exportRolesPermissionsCsv(company, management), `company-${companyId}-roles.csv`);
  } catch (error) { return adminCsvError(error, request, "Role export failed."); }
}
